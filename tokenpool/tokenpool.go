...
//
// Quota-aware 选择 + 401/5xx 死 token 检测。本包供各 Starcat API 与聚合服务共用。
package tokenpool
...
// Count 返回池中 token 总数，用于限制单次 GitHub 请求的最大换 token 次数。
...
// EarliestAvailable 返回最早可恢复的时间（discovery 在全部 token 不可用时等待用）。
// 与 EarliestReset 类似，但若任一 token 已可用则返回零值。
